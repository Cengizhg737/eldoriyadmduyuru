  ...
  if (message.content.toLowerCase().startsWith(prefix + "help".toLowerCase())) {
    ...
    let help = new Discord.MessageEmbed()
      ...
      .setAuthor(
        ...
        message.author.displayAvatarURL({ dynamic: true }) //eldoriya project
      )
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "cd")) {
    ...
    let noargs = new Discord.MessageEmbed()
      ...
      .addField(`Error :x:`, `Lütfen duyurmak istediğiniz mesajı yazınız ! Örnek: +cd Eldoriya Project Açıldı`)
      .setTimestamp()
    if (!args) return message.channel.send(message.author, noargs); //eldoriya project
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "ocd")) {
    if (!message.member.hasPermission("ADMINISTRATOR")) return; //eldoriya project
